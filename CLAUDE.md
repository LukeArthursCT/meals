...
1. Create `data/recipes/{recipe-id}.json` with full recipe details including `id`, `name`, `mealType` (breakfast|lunch|snack|dinner), `tags`, `nutrition` (calories, protein, carbs, fiber, fat), `servings`, `prepTime`, `cookTime`, `ingredientSections`, `steps`, and `dateAdded`.
2. Add a matching summary entry to `data/recipes-index.json` with `id`, `name`, `mealType`, `tags`, `calories`, `protein`, `fiber`, `servings`, `prepTime`, and `cookTime`.
